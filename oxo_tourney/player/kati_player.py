...
def check_win_board(board, symbol, size):
    return_value: int = 0
    return_value += check_rows(board, symbol, size)
    return_value += check_columns(board, symbol, size)
    return_value += check_diagonals(board, symbol, size)
    return return_value
...
def check_win_move(spaces, board, size, symbol):
    for var in spaces:
        ...
        temp = check_win_board(board_copy, symbol, size)
        board_copy[var] = "."
        if temp != 0:
            ret = var
            return ret
...
def get_position(board, symbol, size):
    ...
    ret = check_win_move(available_spaces, board_state, size, symbol)
    if ret != 0xFF:
        return ret

    # block a win
    other_symbol = get_other_symbol(symbol)
    ret = check_win_move(available_spaces, board_state, size, other_symbol)
    if ret != 0xFF:
        return ret
